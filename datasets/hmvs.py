...
from utils.pose_utils import align_global, get_center, normalize_poses, create_spheric_poses, bin2cams
...
class ColmapDatasetBase():
    ...
    def setup(self, config, split):
        ...
        if not ColmapDatasetBase.initialized:
            self.pose_path = os.path.join(self.config.root_dir,
                                          self.config.pose_file)
            assert os.path.isfile(self.pose_path)
            if self.config.pose_file.endswith('.bin'):
                all_c2w, intrinsics, fns = bin2cams(self.pose_path)
                ...
                try:
                    ...
                    print(all_c2w[()]['FRONT'].shape()[0], 'images')
                    try:
                        all_c2w = np.concatenate(
                            (all_c2w[()]['FRONT'], all_c2w[()]['FRONT_LEFT'],
                             all_c2w[()]['FRONT_RIGHT']),
                            axis=0)
                        intrinsics = np.concatenate(
                            (intrinsics[()]['FRONT'],
                             intrinsics[()]['FRONT_LEFT'],
                             intrinsics[()]['FRONT_RIGHT']),
                            axis=0)
                        print('Using', colored('front 3', 'blue'), 'cameras')
                    except:
                        all_c2w = all_c2w[()]['FRONT']
                        intrinsics = intrinsics[()]['FRONT']
                        print('Using', colored('front', 'blue'),
                              'camera alone')
                except:
                    # LiAuto dataset
                    print('Using', colored('all', 'blue'), 'available cameras')
                fns = sorted(
                    os.listdir(os.path.join(self.config.root_dir, 'images')))
                fns = [
                    os.path.join(self.config.root_dir, 'images', fn)
                    for fn in fns
                ]
            ...
            vis_mask_ori_dir = os.path.join(self.config.root_dir, f'masks')
            vis_mask_dir = os.path.join(self.config.root_dir,
                                        f'masks_{self.config.img_downscale}')
            # masks labling dynamic objects
            dyn_mask_ori_dir = os.path.join(self.config.root_dir,
                                                f'moving_vehicle_bound')
            dyn_mask_dir = os.path.join(
                self.config.root_dir,
            ...
            for i, d in enumerate(all_c2w):
                ...
                if self.split in ['train', 'val']:
                    ...
                    img = TF.pil_to_tensor(img).permute(1, 2, 0)[
                        ..., :
                        3] / 255.0  # (4, h, w) => (h, w, 4 ) and normalize it
                    img = img.to(
                        self.rank
                    ...
                    vis_mask_path = os.path.join(
                        vis_mask_dir,
                        'mask' + fns[i].split("/")[-1][6:-3] + 'png')
                    if not os.path.exists(vis_mask_path) and os.path.exists(
                            vis_mask_path.replace(f"{vis_mask_dir}",
                                                  f"{vis_mask_ori_dir}")):
                        os.makedirs(os.path.dirname(vis_mask_path),
                                    exist_ok=True)
                        vis_mask_path = vis_mask_path.replace(
                            f"{vis_mask_dir}", f"{vis_mask_ori_dir}")
                    if os.path.exists(vis_mask_path):
                        self.num_cams = np.max([
                            self.num_cams,
                            int(fns[i].split("/")[-1][7:-4]) + 1
                        ])
                        vis_mask = Image.open(vis_mask_path)
                        if vis_mask.size[0] != w or vis_mask.size[1] != h:
                            vis_mask = vis_mask.resize(img_wh, Image.NEAREST)
                            vis_mask_path = vis_mask_path.replace(
                                f"{vis_mask_ori_dir}", f"{vis_mask_dir}")
                            vis_mask.save(vis_mask_path)
                        vis_mask = TF.to_tensor(vis_mask)[0]
                    else:
                        vis_mask = torch.ones_like(img[..., 0],
                                                   device=img.device)

                    # NOTE: dynamic object mask
                    dyn_mask_path = os.path.join(
                        dyn_mask_dir, fns[i].split("/")[-1][:-3] + 'png')
                    if not os.path.exists(dyn_mask_path):
                        os.makedirs(os.path.dirname(dyn_mask_path),
                                    exist_ok=True)
                        dyn_mask_path = dyn_mask_path.replace(
                            f"{dyn_mask_dir}", f"{dyn_mask_ori_dir}")
                    if os.path.exists(dyn_mask_path):
                        dyn_mask = Image.open(dyn_mask_path)
                        if dyn_mask.size[0] != w or dyn_mask.size[
                                1] != h:
                            dyn_mask = dyn_mask.resize(
                                img_wh, Image.NEAREST)
                            dyn_mask_path = dyn_mask_path.replace(
                                f"{dyn_mask_ori_dir}",
                                f"{dyn_mask_dir}")
                            dyn_mask.save(dyn_mask_path)
                        vis_mask *= (1 - TF.to_tensor(dyn_mask)[0])

                        # save maskes for nerfstudio training
                    ...
                    if self.config.apply_depth and os.path.exists(
                            os.path.dirname(fns[i].replace(
                                f"{img_folder}", f"/{depth_folder}"))):
                        ...
                        if not os.path.exists(depth_path):
                            os.makedirs(os.path.dirname(depth_path),
                                        exist_ok=True)
                            depth_path = fns[i].replace(
                                f"{img_folder}", f"/{depth_folder}").replace(
                                    ".png", depth_format)
                        # loading depth
                    ...
                    if depth.max() != 0.0:
                        ...
                        depth_o3d = o3d.geometry.Image(
                            depth.numpy() *
                            (1 - (np.array(dyn_mask) / 255.0).astype(
                        ...
                        pts_frm = o3d.geometry.PointCloud.create_from_depth_image(
                            depth_o3d,
                            intrinsic=intrin_o3d,
                            extrinsic=np.linalg.inv(all_c2w[i]),
                            depth_scale=1)
                        o3d.io.write_point_cloud(
                            os.path.join(
                                mesh_dir,
                                f"layout_depth_frame_{str(i + 1)}.ply"),
                            pts_frm)
                        pts_clt.points = (o3d.utility.Vector3dVector(
                            np.concatenate((np.array(
                                pts_clt.points), np.array(pts_frm.points)),
                                           axis=0)))

                    all_vis_masks.append(vis_mask)
                    all_images.append(img)

            print('Training with', colored(self.num_imgs, 'blue'), 'shot by',
                  colored(self.num_cams, 'blue'), 'cameras')
            # pts_clt = pts_clt.voxel_down_sample(voxel_size=0.01)
            o3d.io.write_point_cloud(
                os.path.join(mesh_dir, 'layout_depth_clt.ply'), pts_clt)
            ...
            if os.path.exists(path_pts_target):
                ...
                if not os.path.exists(os.path.join(mesh_dir, 'pose_crt.npy')):
                    ...
                    reg_p2p_trans = np.load(
                        os.path.join(mesh_dir, 'pose_crt.npy'))
                all_c2w = np.matmul(reg_p2p_trans, all_c2w)
            else:
                print(
                    colored('Skip aligning to other given coordinates, continue',
                            'green'))
            ...
            elif self.config.pcd_file:
                self.pcd_path = os.path.join(self.config.root_dir,
                                              self.config.pcd_file)
                assert os.path.isfile(self.pcd_path)
                print(
                    colored('Point cloud loaded with a given fused cloud',
                            'green'))
                if self.config.pcd_file.endswith('.npz'):
                    pts3d = []
                    pts3d_frames = np.load(
                        self.pcd_path,
                        allow_pickle=True)['pointcloud'].item()
                    for id_pts, pts_frame in pts3d_frames.items():
                        pts3d.append(np.array(pts_frame))
                    pts3d = np.vstack(pts3d)[:, :3]
                elif self.config.pcd_file.endswith('.ply'):
                    pts3d = np.asarray(
                        o3d.io.read_point_cloud(self.pcd_path).points)
            else:
            ...
            if pts3d is not None:
                ...
                if not os.path.exists(
                        os.path.join(mesh_dir, 'layout_pcd_gt.ply')):
                    # pcd = pcd.voxel_down_sample(voxel_size=0.002)
                    pcd.estimate_normals(
                        search_param=o3d.geometry.KDTreeSearchParamHybrid(
                            radius=0.05, max_nn=20))
                    o3d.io.write_point_cloud(
                        os.path.join(mesh_dir, 'layout_pcd_gt.ply'), pcd)
                else:
                    pcd.normals = o3d.io.read_point_cloud(
                        os.path.join(mesh_dir, 'layout_pcd_gt.ply')).normals
                """
                ...
                if os.path.exists(path_pts_target):
                    print(
                        colored(
                            'Rasterizing on mesh presented in target coordinates',
                            'green'))
                    if os.path.exists(
                            os.path.join(mesh_dir,
                                         'layout_depth_clt_transform.ply')):
                        pcd = o3d.io.read_point_cloud(
                            os.path.join(mesh_dir,
                                         'layout_depth_clt_transform.ply'))
                        mesh_poisson_path = os.path.join(
                            mesh_dir, 'layout_mesh_ps_transform.ply')
                    else:
                        pcd = o3d.io.read_point_cloud(path_pts_target)
                        mesh_poisson_path = os.path.join(
                            mesh_dir, 'layout_mesh_ps_gt.ply')
                else:
                    mesh_poisson_path = os.path.join(mesh_dir,
                                                     'layout_mesh_ps.ply')
                    # mesh_poisson_path = os.path.join(mesh_dir, 'layout_mesh_nksr.ply')
                    print(
                        colored(
                            'Rasterizing on mesh presented in original coordinates',
                            'green'))
                    pcd = o3d.io.read_point_cloud(
            ...
            for i, d in enumerate(all_c2w):
                if self.split in ['train', 'val']:
                    if self.config.apply_depth:
                        pts_clt = o3d.t.geometry.PointCloud.from_legacy(
                            o3d.io.read_point_cloud(
                                os.path.join(mesh_dir,
                                             'layout_depth_clt.ply')))
                        depth_reproj = pts_clt.project_to_depth_image(
                            w,
                            h,
                            intrinsics[i],
                            ...
                            depth_scale=1.0,
                            depth_max=dep_max)
                        depth_reproj = Image.fromarray(
                            np.asarray(depth_reproj.to_legacy()))
                        ...
                        rays_mesh = scene.create_rays_pinhole(
                            ...
                            extrinsic_matrix=np.linalg.inv(all_c2w[i]),
                            width_px=w,
                            height_px=h)
                    ...
                    if self.config.apply_mask:
                        ...
                        if os.path.exists(mask_path):
                            ...
                            mask = TF.to_tensor(
                                mask
                            )[0]  # TF.pil_to_tensor does not rescale the input PIL mask
                            mask_fg = torch.ones_like(mask, device=img.device)
                            mask_fg[mask ==
                                    1] = 0  # check whether we use 1 or 255
                            mask = mask_fg
                            ...
                            print(
                                colored(
                                    'Foreground mask not available, need to disable apply_mask',
                                    'red'))
                        if mask.shape[-1] == 1:
                            mask = mask[..., 0]
                    else:
                    ...
                    if self.config.apply_depth:
                        ...
                        np.save(
                            os.path.join(norm_rast_path,
                                         fns[i].split("/")[-1][:-3] + 'npy'),
                            norm_rast)

                        save_norm_dep_vis = True
                        depth_rast_quantize = Image.fromarray(
                            depth_rast.astype(np.uint16))
                        ...
                        if save_norm_dep_vis:
                            ...
                            depth_rast.save(
                                os.path.join(
                                    depth_rast_path,
                                    fns[i].split("/")[-1][:-3] + 'tiff'))
                            depth_rast_quantize.save(
                                os.path.join(
                                    depth_rast_path,
                                    fns[i].split("/")[-1][:-3] + 'png'))

                            # save rasterized norm
                            norm_rast = Image.fromarray(
                                ((norm_rast + 1) * 128).astype(np.uint8))
                            norm_rast_path = os.path.join(
                                self.config.root_dir,
                                f"norm_rast_{self.config.img_downscale}")
                            os.makedirs(norm_rast_path, exist_ok=True)
                        ...
                        depth_rast_mask = (depth_rast > 0.0).to(
                            bool
                        )  # trim points outside the contraction box off
        ...
        if self.split == 'test':
            ...
            self.all_c2w = self.all_c2w[
                n_steps * self.num_cams]  # front alone when there is 6 cameras
            ...
            self.all_images, self.all_vis_masks, self.all_fg_masks = torch.stack(
                self.all_images, dim=0).float(), torch.stack(
                    self.all_vis_masks, dim=0).float(), torch.stack(
                        self.all_fg_masks, dim=0).float()
            if len(self.all_depths) > 0:
                self.all_depths, self.all_depth_masks = torch.stack( self.all_depths, dim=0).float(), torch.stack(self.all_depth_masks, dim=0).bool()
        """
        # for debug use
